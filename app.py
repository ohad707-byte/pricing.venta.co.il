...
DEFAULT_PATTERNS = [
    ...
    ("תעלה מלבנית", r"(?<!\d)(?:[6-9]0|1[0-9]0|2[0-9]0|3[0-9]0|400)\s*/\s*(?:[2-9]0|1[0-5]0)(?!\d)", "יח'"),
...
def estimate_duct_measurements(pdf_results: List[dict], pricing: pd.DataFrame, margin: float, rect_m_per_label: float, round_m_per_label: float, waste_pct: float) -> pd.DataFrame:
    ...
    for res in pdf_results:
        ...
        rect_matches = [m.group(0) for m in re.finditer(r"(?<!\d)(?:[6-9]0|1[0-9]0|2[0-9]0|3[0-9]0|400)\s*/\s*(?:[2-9]0|1[0-5]0)(?!\d)", text, flags=re.I)]
...
# עדכון מהיר של קוביות התעלות מתוך הסיידבר
for key, buy_val, sell_val in [
    ("DUCT_SHEET_M2", duct_sheet_buy_quick, duct_sheet_sell_quick),
    ("DUCT_ROUND_M", duct_round_buy_quick, duct_round_sell_quick),
]:
    mask = st.session_state["cost_blocks"]["key"].astype(str).eq(key)
    if mask.any():
        if buy_val > 0:
            st.session_state["cost_blocks"].loc[mask, "buy_price"] = buy_val
        if sell_val > 0:
            st.session_state["cost_blocks"].loc[mask, "sell_price"] = sell_val
...
if len(st.session_state["items"]):
    ...
    if len(duct_view):
        duct_qty = pd.to_numeric(duct_view["כמות לתמחור"], errors="coerce").fillna(0).sum()
        duct_sell = pd.to_numeric(duct_view["סהכ מכירה"], errors="coerce").fillna(0).sum()
        st.metric("כמות תעלות שנמצאה", f"{duct_qty:,.1f}")
        if duct_sell == 0:
